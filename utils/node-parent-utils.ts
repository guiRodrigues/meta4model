...
export const getNodePositionInsideParent = (
  ...
  const nodeWidth = (node as any).measured?.width ?? 0;
  const nodeHeight = (node as any).measured?.height ?? 0;
  const groupWidth = (groupNode as any).measured?.width ?? 0;
  const groupHeight = (groupNode as any).measured?.height ?? 0;
