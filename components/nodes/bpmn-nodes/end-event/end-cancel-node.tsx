...
import { CircleEndCancelIcon } from "@/components/icons/bpmn-icons";
import useDetachNodes from "@/hooks/use-detach-nodes";

function EndCancelEventNode({ data, type, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    setIsEditing(false);
    data.label = label.trim() || "Cancel End";
  };
  ...
  return (
    ...
    >
      <div className="pt-0 pb-0">
        <CircleEndCancelIcon className={`h-${size} w-${size} text-red-500`} />
      </div>
...
export default memo(EndCancelEventNode);
