...
import { CircleEndMultipleIcon } from "@/components/icons/bpmn-icons";
import useDetachNodes from "@/hooks/use-detach-nodes";

function EndMultipleEventNode({ data, type, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    setIsEditing(false);
    data.label = label.trim() || "Multiple End";
  };
  ...
  return (
    ...
    >
      <div className="pt-0 pb-0">
        <CircleEndMultipleIcon className={`h-${size} w-${size} text-red-500`} />
      </div>
...
export default memo(EndMultipleEventNode);
