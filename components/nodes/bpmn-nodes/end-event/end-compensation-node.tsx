...
import { CircleEndCompensationIcon } from "@/components/icons/bpmn-icons";
import useDetachNodes from "@/hooks/use-detach-nodes";

function EndCompensationEventNode({ data, type, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    setIsEditing(false);
    data.label = label.trim() || "Compensation End";
  };
  ...
  return (
    ...
    >
      <div className="pt-0 pb-0">
        <CircleEndCompensationIcon className={`h-${size} w-${size} text-red-500`} />
      </div>
        ...
        {isEditing ? (
          <div className="flex justify-center mt-1">
            <input
              ...
              className="w-4/5 bg-white/80 text-xs font-medium outline-none border-b border-red-700 text-center text-gray-700 rounded px-1"
            />
          </div>
        ) : (
          <div
...
export default memo(EndCompensationEventNode);
