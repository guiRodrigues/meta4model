...
import { CircleEndMessageIcon } from "@/components/icons/bpmn-icons";
import useDetachNodes from "@/hooks/use-detach-nodes";

function EndMessageEventNode({ data, type, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    setIsEditing(false);
    data.label = label.trim() || "Message End";
  };
  ...
  return (
    ...
    >
      <div className="pt-0 pb-0">
        <CircleEndMessageIcon className={`h-${size} w-${size} text-red-500`} />
      </div>
...
export default memo(EndMessageEventNode);
