...
import { CircleEndEscalationIcon } from "@/components/icons/bpmn-icons";
import useDetachNodes from "@/hooks/use-detach-nodes";

function EndEscalationEventNode({ data, type, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    setIsEditing(false);
    data.label = label.trim() || "Escalation End";
  };
  ...
  return (
    ...
    >
      <div className="pt-0 pb-0">
        <CircleEndEscalationIcon className={`h-${size} w-${size} text-red-500`} />
      </div>
...
export default memo(EndEscalationEventNode);
