...
import { CircleEndTerminateIcon } from "@/components/icons/bpmn-icons";
import useDetachNodes from "@/hooks/use-detach-nodes";

function EndTerminateEventNode({ data, type, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    setIsEditing(false);
    data.label = label.trim() || "Terminate";
  };
  ...
  return (
    ...
    >
      <div className="pt-0 pb-0">
        <CircleEndTerminateIcon className={`h-${size} w-${size} text-red-600`} />
      </div>
...
export default memo(EndTerminateEventNode);
