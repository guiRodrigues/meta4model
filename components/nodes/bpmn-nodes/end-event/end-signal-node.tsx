...
import { CircleEndSignalIcon } from "@/components/icons/bpmn-icons";
import useDetachNodes from "@/hooks/use-detach-nodes";

function EndSignalEventNode({ data, type, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    setIsEditing(false);
    data.label = label.trim() || "Signal End";
  };
  ...
  return (
    ...
    >
      <div className="pt-0 pb-0">
        <CircleEndSignalIcon className={`h-${size} w-${size} text-red-500`} />
      </div>
...
export default memo(EndSignalEventNode);
