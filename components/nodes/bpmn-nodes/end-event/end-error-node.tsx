...
import { CircleEndErrorIcon } from "@/components/icons/bpmn-icons";
import useDetachNodes from "@/hooks/use-detach-nodes";

function EndErrorEventNode({ data, type, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    setIsEditing(false);
    data.label = label.trim() || "Error End";
  };
  ...
  return (
    ...
    >
      <div className="pt-0 pb-0">
        <CircleEndErrorIcon className={`h-${size} w-${size} text-red-500`} />
      </div>
...
export default memo(EndErrorEventNode);
