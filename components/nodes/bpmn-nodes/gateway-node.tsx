...
import { DiamondExclusiveIcon, DiamondIcon, DiamondParallelIcon } from "@/components/icons/bpmn-icons"
...
function GatewayNode({ data, type, id }: NodeProps<NodeData>) {
    ...
    function renderIconByType() {
        switch (type) {
            case "exclusive":
                return <DiamondExclusiveIcon className={`h-${size} w-${size} text-grey-500`} />
            case "parallel":
                return <DiamondParallelIcon className={`h-${size} w-${size} text-grey-500`} />
            default:
