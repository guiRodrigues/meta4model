...
import { DiamondEventBasedIcon } from "@/components/icons/bpmn-icons";
import useDetachNodes from "@/hooks/use-detach-nodes";

function GatewayEventBasedNode({ data, type, id }: NodeProps<NodeData>) {
  ...
  const handleBlur = () => {
    setIsEditing(false);
    data.label = label.trim() || "Event-Based Gateway";
  };
  ...
  return (
    ...
    >
      <div className="pt-0 pb-0">
        <DiamondEventBasedIcon className={`h-${size} w-${size} text-green-500`} />
      </div>
...
export default memo(GatewayEventBasedNode);
